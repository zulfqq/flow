...
pub(super) struct Actor<P: crate::Publisher, L: crate::Logger> {
    ...
    logger: L,
    // True only for an unsplit capture's single shard (full key- and r-clock
    // range). Gates backfill truncation: only this shard may emit backfill
    // messages and manage truncated-at labels, as a sub-range shard can't
    // observe a backfill's full lifecycle.
    ...
    fn dispatch(
        ...
        let blocking = match action {
            ...
            fsm::Action::Captured { captured } => {
                ...
                memtable.add(binding as u16, doc, false, 0)?;
                true
            }
            ...
            fsm::Action::Checkpoint { checkpoint } => {
                ...
                }) = checkpoint.state
                {
                    ...
                    memtable.add(self.task.bindings.len() as u16, doc, false, 0)?;
                }
            ...
            fsm::Action::ApplyTruncatedLabels => {
                // Only a single-shard capture manages truncated-at labels. A
            ...
            if !self.is_single_shard {
                anyhow::bail!(
                    "connector emitted BackfillBegin for binding {binding}, but \
                     backfill truncation requires an unsplit (single-shard) capture"
                );
            }
...
mod tests {
    ...
    /// A sub-range shard (not a single-shard capture) that inherited
    /// `active_backfills` via a mid-backfill split must NOT apply truncated-at
    /// labels — only a single-shard capture manages them, and a sub-range shard
    /// never receives the BackfillComplete that would clear them.
    #[tokio::test]
    async fn non_single_shard_skips_label_apply() {
        let (connector_tx, _connector_rx) = mpsc::channel::<Request>(crate::CHANNEL_BUFFER);
        let task = std::sync::Arc::new(mk_task(true));
        let publisher = crate::publish::NoopPublisher;
        let shapes = task.binding_shapes_by_index(Default::default());
        ...
        let mut actor = Actor::new(
            ...
            connector_tx,
            crate::shard::RocksDB::open(None).await.unwrap(),
            false, // NOT a single-shard capture
            super::super::Metrics::new("test/shard"),
            crate::TracingLogger,
            publisher,
            shapes,
        ...
        assert!(
            ...
            "a split (non-single-shard) capture must not apply truncated-at labels",
        );
    }
