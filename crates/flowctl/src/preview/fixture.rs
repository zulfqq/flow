//! Fixture *file format* for `flowctl preview --fixture`.
//!
//! This is the flowctl-side half of fixture preview: it parses the
//! newline-delimited fixture file, plans transactions across `--sessions`, and
//! feeds the shared segment writer in `runtime_harness::drive::segments`. The
//! lower half — writing shuffle log segments, stamping clocks, and the
//! channel-fed [`ShuffleSessionFactory`](runtime_next::ShuffleSessionFactory)
//! that relays their checkpoint frontiers — lives in the shared drive layer, so
//! preview and the catalog-test runner produce byte-identical segments.
//!
...
use runtime_harness::drive::segments::{self, FixtureItem, Transaction, write_transaction};
...
struct Keepalive {
    _writers: Vec<segments::ShardWriter>,
...
pub fn build(
    ...
    // Preview is single-shard, so the routing topology is one full-range shard.
    let shards = segments::full_range_shards(1);
    let mut clock = uuid::Clock::from_unix(1, 0);
    let mut journal_offsets: HashMap<(String, u16), i64> = HashMap::new();
    let mut round_robin: HashMap<u16, usize> = HashMap::new();
    ...
    for (session_index, &budget) in session_targets.iter().enumerate() {
        ...
        let mut writers = vec![segments::ShardWriter::new(std::path::Path::new(&dir), 0)?];
...
pub fn start_streaming(
    ...
    let writers = vec![segments::ShardWriter::new(std::path::Path::new(&dir), 0)?];
...
async fn feed_stream(
    ...
    path: Option<std::path::PathBuf>,
    mut writers: Vec<segments::ShardWriter>,
    frontier_tx: tokio::sync::mpsc::UnboundedSender<FixtureItem>,
    eof_stop: tokio_util::sync::CancellationToken,
    hold: tokio_util::sync::CancellationToken,
) -> anyhow::Result<()> {
...
async fn feed_lines(
    ...
    collection_bindings: &HashMap<String, Vec<usize>>,
    path: Option<std::path::PathBuf>,
    writers: &mut [segments::ShardWriter],
