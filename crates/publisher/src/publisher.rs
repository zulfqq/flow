...
impl Publisher {
    ...
    pub async fn apply_truncated_at_labels(
        ...
        for (&index, &clock) in active_backfills {
            ...
            // race. Partitions already at the target are skipped on the retry
            // and the loop converges once any racing writer (a concurrent split
            // or activation) settles. Bounded at 10 attempts so a persistently-
            // racing writer (or a bug that keeps losing the CAS) fails the
            // transaction rather than spinning forever and silently wedging the
            // caller.
            for cas_retries in 0..10 {
                ...
                if cas_retries == 9 {
                    return Err(tonic::Status::aborted(format!(
                        "applying truncated-at label to {} lost race 10 times in a row",
                        binding.collection,
                    )));
                }
                tracing::warn!(
                    ...
                    cas_retries,
                    "truncated-at label apply lost race; will retry",
                );
            }
        }
        Ok(())
